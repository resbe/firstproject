...
//	}
	...
	// 책 수정 1.제목 수정 | 2.저자 수정 | 3.장르 수정 | 4.가격 수정 |
	// 책 이름 수정
	public int bookUpdate1(Book book,int choice) {
		int result = 0;
		try {
			conn();
	...
	public int bookUpdate2(Book book,int choice) {
		int result = 0;
		try {
			conn();
			String sql = "update book set bookWriter = ? where bookId = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public int bookUpdate3(Book book,int choice) {
		int result = 0;
		try {
			conn();
			String sql = "update book set bookGenre = ? where bookId = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public int bookUpdate4(Book book,int choice) {
		int result = 0;
		try {
			conn();
			String sql = "update book set bookPriced = ? where bookId = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, book.getBookPriced());
			pstmt.setInt(2,choice);
